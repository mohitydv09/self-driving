...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    # 3.1 Compute Kalman Gain ## K_h = P H (H P H + R)-1
    sensor_var = np.eye(3) * sensor_var
    K_k = p_cov_check @ h_jac.T @ np.linalg.inv(h_jac @ p_cov_check @ h_jac.T + sensor_var)

    # 3.2 Compute error state ## delx = K(y - h(x))
    del_x = K_k @ (y_k - p_check)

    # 3.3 Correct predicted state ## x = x + delx
    p_hat = p_check + del_x[:3]
    v_hat = v_check + del_x[3:6]
    q_hat = Quaternion(axis_angle=del_x[6:]).quat_mult_right(q_check)
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    # 1. Update state with IMU inputs ## x = f(x,u)
    ...
    # 2. Propagate uncertainty ## P = F P F^T + L Q L^T
    F_k_1 = np.eye(9)
    F_k_1[:3, 3:6] = delta_t * np.eye(3)
    F_k_1[3:6, 6:] = - C_ns @ skew_symmetric(imu_f.data[k-1].reshape((3,1))) #-skew_symmetric(C_ns @ imu_f.data[k-1]) * delta_t
    Q_k_1 = np.eye(6)
    Q_k_1[:, :3] *= var_imu_f * delta_t ** 2
    Q_k_1[:, -3:] *= var_imu_w * delta_t ** 2
